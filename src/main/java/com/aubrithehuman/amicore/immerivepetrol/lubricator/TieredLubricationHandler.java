package com.aubrithehuman.amicore.immerivepetrol.lubricator;
...
//		System.out.println("attempt2");
	...
	public void tierProcess(Fluid fluid, Iterator<MultiblockProcess<R>> processIterator, MultiblockProcess<R> process, L mbte, int ticks) {
		if(fluid.is(FluidTagsAMI.LUBE_TIER_1)) {
			if(ticks % AMIConfig.tier_1_factor.get() == 0) {
//				System.out.println("tier 1 process");
				process(processIterator, process, mbte);
			}
		} else if(fluid.is(FluidTagsAMI.LUBE_TIER_2)) {
			if(ticks % AMIConfig.tier_2_factor.get() == 0) {
//				System.out.println("tier 2 process");
				process(processIterator, process, mbte);
			}
		} else if(fluid.is(FluidTagsAMI.LUBE_TIER_3)) {
			if(ticks % AMIConfig.tier_3_factor.get() == 0){
//				System.out.println("tier 3 process");
				process(processIterator, process, mbte);
			}
		}
	}
